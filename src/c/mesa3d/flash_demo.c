...
Still getting undefined sym: _mesa_calloc

It looks like its occuring in OSMesaCreateContextExt() at the calloc macro.
...
{ 
  ...
  if (!ctx) {
    trace("OSMesaCreateContext failed!\n");
    return 0;
  }
  ...
  if (!buffer) {
    trace("Alloc image buffer failed!\n");
    return 0;
  }
  ...
  if (!OSMesaMakeCurrent( ctx, buffer, GL_UNSIGNED_BYTE, Width, Height )) {
    trace("OSMesaMakeCurrent failed!\n");
    return 0;
  }
  ...
  
  {
    ...
    // trace("Depth=%d Stencil=%d Accum=%d\n", z, s, a);
  }
